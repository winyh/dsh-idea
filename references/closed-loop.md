# dsh-idea：闭环接收与反馈使用契约

这是公开使用说明，不包含私有 `docs/` 实现思路。各插件独立维护自己的接收约定，不引入第七个插件或共享运行时契约包。

## 本插件的接收入口

工具：`idea_handoff_receive`。只读输入，返回回执；不执行下一步动作、不自动写文件，也不触发外部系统。

| 输入工件 | 来源 | 接收方式 | 接收后的下一工具 |
| --- | --- | --- | --- |
| `sales-feedback-handoff` | `dsh-sales` | 定向接收 | `idea_evidence_score` |

关键业务字段：

- `sales-feedback-handoff`：`source`、`feedback`。把失单与市场问题作为新发现线索，补访谈和反证，不把原因频次当成需求验证。
...
接收销售反馈后，回执直接附带可供 `idea_evidence_score` 消费的 `signals`。原因频次保留为聚合线索；不伪造用户原话、用户身份、独立来源或“已验证需求”。
