...
function parseFrontmatter(text$1) {
	const match = text$1.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);
	if (!match) return {
		frontmatter: {},
		body: text$1
	};
...
function parseNote(path, text$1) {
	const { frontmatter, body } = parseFrontmatter(text$1);
...
function parseCsv(text$1, delimiter = ",") {
	const rows = [];
	let row = [];
	let cell = "";
	let quoted = false;
...
function parseRecords(path, text$1) {
	...
	if (extension$1 === "json") try {
		return {
			records: objectRecords(JSON.parse(text$1)),
			warnings: []
		};
	} catch {
		return {
	...
	if (extension$1 === "md" || extension$1 === "mdown" || extension$1 === "markdown") {
		const note = parseNote(path, text$1);
		return {
...
async function readRecords(fs, config, path, signal) {
	...
	if (text$1.length > config.maxTextChars) throw new Error(`Signal source exceeds maxTextChars (${config.maxTextChars}): ${path}`);
	return {
		source: path,
		...parseRecords(path, text$1)
	};
}
...
function stableId(text$1, index) {
	let hash = 2166136261;
	for (const character of text$1) {
...
function includes(text$1, query) {
	return Boolean(text$1?.toLocaleLowerCase().includes(query));
}
...
var IdeaDiscoveryService = class extends Service {
	...
	async importSignals(path, signal) {
		...
		const normalized$1 = normalizeSignals(imported.records, path);
		return {
			source: path,
			signals: normalized$1.signals,
			warnings: [...imported.warnings, ...normalized$1.skipped > 0 ? [`Skipped ${normalized$1.skipped} rows without a problem/pain field.`] : []],
			rowsRead: imported.records.length,
			rowsAccepted: normalized$1.signals.length
		};
	}
...
async function appendArtifactAudit(fs, root, event, signal) {
	const path = join(root, ".dsh-idea-audit.jsonl");
	const target = await fs.resolve(path, { signal });
	const info = await fs.stat(target, signal);
...
const handoffRoutes = { "sales-feedback-handoff": {
	"from": "dsh-sales",
...
function receiveHandoff(value, options) {
	...
	const route = Object.hasOwn(handoffRoutes, sourceType) ? handoffRoutes[sourceType] : void 0;
	...
	if ([
		...
		"stale",
		"rejected"
	].includes(text(source.status)) || ["hold", "stop"].includes(text(source.decision))) issues.push("上游工件要求停止或重新审查，不能向下推进。");
	if ([
		"partial",
		"draft",
		"open",
		"implemented",
		"accepted"
	].includes(text(source.status))) missing.push("上游尚未完成验证");
	...
	return {
		signals: status === "accepted" && sourceType === "sales-feedback-handoff" ? normalizeSignals(source.feedback.map((value$1) => {
			const row = asRecord(value$1);
			return {
				problem: row.reason,
				...
				notes: `聚合销售反馈；来源工件 ${String(source.artifactId)}。频次不是独立访谈、原话或需求验证。`
			};
		}), text(source.source)).signals : [],
		...
		status,
		sourceArtifactId: text(source.artifactId),
		sourceArtifactType: sourceType,
		sourceContentHash: text(source.contentHash),
		sourceProducer: text(source.handoffFrom),
...
function renderResult(value, maxChars) {
	...
	return [{
		type: "text",
		text: text$1.length > maxChars ? `${text$1.slice(0, maxChars)}\n... result truncated by dsh-idea; use a narrower filter or source ...` : text$1
	}];
}
...
function themeForCandidate(review, candidate) {
	return review.opportunityMap.themes.find((theme) => theme.id === candidate?.themeId) ?? review.opportunityMap.themes[0];
}
function yaml(value) {
	return JSON.stringify(value);
}
function markdownList(values, fallback = "待补充") {
	return values.length > 0 ? values.map((value) => `- ${value}`).join("\n") : `- ${fallback}`;
}
function buildOpportunityHandoff(review) {
	...
	const status = candidate && evidence.length > 0 && Boolean(review.experiment) ? "ready" : "partial";
	const nextActions = status === "ready" ? ["交给 dsh-product 生成产品 Brief；不要在产品阶段重新做外部需求发现。", "把验证实验的成功/失败阈值和原始证据写回机会记录。"] : ["补齐候选方向、来源证据和最小验证实验，再交给 dsh-product。"];
	...
	const handoff = {
		...
		artifactId: createArtifactId({
			...
			source: review.source,
			riskiestAssumption,
			validationExperiment: review.experiment
		}),
		handoffVersion: "1.0",
		artifactType: "opportunity-handoff",
...
//#endregion
//#region src/watchlist.ts
function changedFields(current, previous) {
	return [
		"title",
		"description",
		"headings",
...
function buildWatchlistDiff(current, previous = []) {
	...
	const nextActions = items.filter((item) => item.status === "new" || item.status === "changed").length > 0 ? ["审阅 changed/new 页面，再把真实变化转成 signal card；不要把页面变化直接当作需求结论。"] : ["继续保留本次快照，下一周期使用相同 URL 和口径复查。"];
	const markdown = [
		"---",
...
function signalRecordsFromJson(value) {
	...
	const normalized$1 = normalizeSignals(candidate.filter((item) => typeof item === "object" && item !== null && !Array.isArray(item)), "signalsJson");
	return {
		signals: normalized$1.signals,
		warnings: normalized$1.skipped > 0 ? [`Skipped ${normalized$1.skipped} records without a problem/title.`] : []
	};
}
...
function registerIdeaTools(ctx, config, service, fs, web) {
	ctx.tools.register(defineTool({
		...
		parameters: {
			urls: {
				...
				required: true,
				items: { type: "string" },
				description: "One to five public HTTP(S) URLs."
			},
			previousJson: {
				type: "string",
				description: "Optional JSON array or result envelope containing previous external source snapshots."
			}
		},
		...
		async execute(args, exec) {
			...
			if (args.previousJson?.trim()) {
				let parsed;
				try {
					parsed = JSON.parse(args.previousJson);
				} catch (error) {
					throw new Error(`previousJson must be valid JSON: ${error instanceof Error ? error.message : String(error)}`);
				}
				const data = typeof parsed === "object" && parsed !== null && "data" in parsed ? parsed.data : parsed;
				const sources = Array.isArray(data) ? data : typeof data === "object" && data !== null && "sources" in data ? data.sources : [];
				if (!Array.isArray(sources)) throw new Error("previousJson must contain an array of snapshots or a result with sources.");
				previous = sources.filter((item) => typeof item === "object" && item !== null && typeof item.url === "string");
			}
			const fetched = await fetchExternalSources(web, args.urls, config, void 0, exec.signal);
			const result = buildWatchlistDiff(fetched.sources, previous);
			result.warnings.push(...fetched.warnings);
			return wrapResult(result, {
				lineage: fetched.sources.map((item) => ({ source: item.finalUrl ?? item.url })),
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			path: {
				type: "string",
				required: true,
				description: "Opportunity Markdown path under defaultRoot."
			},
			content: {
				type: "string",
				required: true,
		...
		async execute(args, exec) {
			...
			const diff = {
				beforeLines: current.split(/\r?\n/).length,
				afterLines: args.content.split(/\r?\n/).length,
				changed: current !== args.content
			};
			if (!args.confirm) return wrapResult({
				status: "preview-only",
				path: args.path,
				applied: false,
				diff
			}, { nextActions: ["审阅变化；确认后再次调用并设置 confirm=true。"] });
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			return wrapResult({
				artifactType: "idea-artifact-review",
				generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
				...review
