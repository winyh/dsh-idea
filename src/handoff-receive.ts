...
import { normalizeSignals } from './signals.js'
...
export function receiveHandoff(value: unknown, options: ReceiveOptions) {
  ...
  const signals = status === 'accepted' && sourceType === 'sales-feedback-handoff'
    ? normalizeSignals((source.feedback as unknown[]).map(value => {
      const row = asRecord(value)
      return {
        problem: row.reason, source: source.source, sourceType: 'other', repeatCount: row.count,
        evidenceState: 'signal', capturedAt: source.generatedAt,
